...
{
    return [
        'BIF', 'CLP', 'DJF', 'GNF', 'JPY', 'KMF', 'KRW', 'MGA', 'PYG', 'RWF', 'UGX', 'VND', 'VUV', 'XAF', 'XOF', 'XPF', 'NGN',
    ];
}
...
function getMenuLinks($menu)
{
    ...
    if ($menu == User::MAIN_PATIENT_CASE) {
        ...
        $subMenus = ['Patients', 'Cases', 'Case Handlers', 'Patient Admissions', 'Encounter'];
    }
    // Pathology
    ...
    if ($menu == User::MAIN_DOCTOR_PATIENT_CASE) {
        $defaultRoute = route('patients.index');
        $subMenus = ['Patient Admissions', 'Encounter'];
    }
    ...
    if ($menu == User::MAIN_CASE_MANGER_PATIENT_CASE) {
        ...
        $subMenus = ['Cases', 'Patient Admissions', 'Encounter'];
    }
